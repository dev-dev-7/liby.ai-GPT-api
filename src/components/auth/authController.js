...
exports.getUserDetails = async (req, res) => {
  ...
  let auth = await authorization.authorization(req, res)
  if (auth) {
    let user = await authModel.getUserById(auth.user_id);
    return res.status(200).json({
      data: user,
      plan: await authModel.getPlanById(user.plan_id),
      image: await authModel.getDayTokens(user.user_id, 'image'),
      audio: await authModel.getDayTokens(user.user_id, 'audio'),
      video: await authModel.getDayTokens(user.user_id, 'video'),
    })
  } else {
    return res.status(404).json({ errors: [{ msg: 'Not Found' }] })
  }
